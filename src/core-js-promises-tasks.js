...
function getPromise(number) {
  const prom = new Promise((res, rej) => {
    if (number < 0) rej(Error('promise that will be rejected'));
    else res('promise that will be fulfilled');
  });
  return prom;
}
...
function getFirstResolvedPromiseResult(promises) {
  const firtsSuccess = Promise.any(promises);
  return firtsSuccess;
}
...
function getFirstPromiseResult(promises) {
  const firstResOrRej = Promise.race(promises);
  return firstResOrRej;
}
...
function getAllOrNothing(promises) {
  const allProm = Promise.all(promises);
  return allProm;
}
...
function getAllResult(promises) {
  const successedPr = (pr) => {
    return pr.then((res) => res).catch(() => null);
  };
  return Promise.all(promises.map(successedPr));
}
...
function queuPromises(promises) {
  let output = '';
  const handlePromise = (prom, recursFn, i) => {
    return prom.then((res) => {
      output += String(res);
      return recursFn(i + 1);
    });
  };
  const waitTheRes = (idx = 0) => {
    if (idx === promises.length) {
      return Promise.resolve(output);
    }
    return handlePromise(promises[idx], waitTheRes, idx);
  };
  return waitTheRes();
}
